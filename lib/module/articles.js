...
exports.Getlist = function(req, res, next){
    ...
    sort = req.query.sort ? parseInt(req.query.sort) : -1,
		status = req.query.status ? req.query.status : ['publish', 'save'],
		from = req.query.from ? req.query.from : null,
    findParams = {};//筛选
    
  if(from!=='front'){//前台网站展示文章只展示发布的
    findParams = {operate: {"$in": status}};
		if(schWord){//标题，正文，标签内包含关键字(js的RegExp对象)
			var schRegExp = new RegExp(schWord,"i");
			findParams = {"$or":[{'title':schRegExp}, {'text':schRegExp}, {'brief':schRegExp}, {'tags':schRegExp}], operate: {"$in": status}};
		}
	}else{
