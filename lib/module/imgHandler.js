...
exports.Getimglist = function(req, res, next){
    if(typeof req.query.theme === 'string') {
        req.query.theme = JSON.parse(req.query.theme)
    }
    ...
    // 搜索条件组合
    if(theme.length > 0) {
      Object.assign(findParams, {'theme': {"$in": theme}})
    }
    if(schWord){//标题，正文，标签内包含关键字(js的RegExp对象)
      var schRegExp = new RegExp(schWord,"i");
      Object.assign(findParams, { "$or": [{'title':schRegExp}, {'desc':schRegExp}] });//图片搜索只根据图片描述进行搜索
    }
...
exports.MNPimgThemes = function(req, res, next){
	...
	.then((data)=>{
		if(data&&data!=''){//(js的concat()方法)
			...
			// 	themesArr = themesArr.concat(data[i].theme)
			// }
            ...
            //     }
            // }
            allTheme['people'].list = data.filter((el)=>{
                return el.theme === 'people'
            })
            allTheme['sence'].list = data.filter((el)=>{
                return el.theme === 'sence'
            })
            allTheme['pets'].list = data.filter((el)=>{
                return el.theme === 'pets'
            })
            allTheme['food'].list = data.filter((el)=>{
              return el.theme === 'food'
          })
			res.json({
			    res_code:1,
			    themeList: allTheme
	...
	.catch((err)=>{
		console.log('Articles.find---11--err');
		console.log(err);
		switch(err){
			case 2 :
				res.json({
  ...
  Img.find({})
	...
	.then((data)=>{
		if(data&&data!=''){//(js的concat()方法)
			...
			let themeArr = [];
			for(let i=0;i<data.length;i++){
                if (!data[i].theme) continue
				themeArr = themeArr.concat(data[i].theme)
			}
			//数组去重：new Set()去重，Array.from()转换为数组
			let themes = Array.from(new Set(themeArr));
			res.json({
			    res_code:1,
			    themes:themes
			})
